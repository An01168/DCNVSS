    ...
    for i in range(args.epoch_iters):
        ...
        if i % args.disp_iter == 0:
            print('Epoch: [{}][{}/{}], Time: {:.2f}, Data: {:.2f}, '
                  'lr_match: {:.6f}, lr_nonkeydecoder: {:.6f}, '
                  'Accuracy: {:4.2f}, Loss: {:.6f}'
                  .format(epoch, i, args.epoch_iters,
                          batch_time.average(), data_time.average(),
                          args.running_lr_match, args.running_lr_nonkeydecoder,
...
def checkpoint(nets, history, args, epoch_num):
    ...
    (nonkey_match, nonkey_decoder, crit) = nets
    suffix_latest = 'epoch_{}.pth'.format(epoch_num)

    dict_match = nonkey_match.state_dict()
    dict_nonkeydecoder = nonkey_decoder.state_dict()


    torch.save(history,
               '{}/history_{}'.format(args.ckpt, suffix_latest))
    torch.save(dict_match,
               '{}/match_{}'.format(args.ckpt, suffix_latest))
    torch.save(dict_nonkeydecoder,
               '{}/nonkeydecoder_{}'.format(args.ckpt, suffix_latest))
...
def create_optimizers(nets, args):
    (nonkey_match, nonkey_decoder, crit) = nets
    optimizer_match = torch.optim.SGD(
        group_weight(nonkey_match),
        lr=args.lr_match,
        momentum=args.beta1,
        weight_decay=args.weight_decay)
    optimizer_nonkeydecoder = torch.optim.SGD(
        group_weight(nonkey_decoder),
        lr=args.lr_nonkeydecoder,
        momentum=args.beta1,
        weight_decay=args.weight_decay)
    return (optimizer_match, optimizer_nonkeydecoder)


def adjust_learning_rate(optimizers, cur_iter, args):
    scale_running_lr = ((1. - float(cur_iter) / args.max_iters) ** args.lr_pow)
    args.running_lr_match = args.lr_match * scale_running_lr
    args.running_lr_nonkeydecoder = args.lr_nonkeydecoder * scale_running_lr


    (optimizer_match, optimizer_nonkeydecoder) = optimizers
    for param_group in optimizer_match.param_groups:
        param_group['lr'] = args.running_lr_match
    for param_group in optimizer_nonkeydecoder.param_groups:
        param_group['lr'] = args.running_lr_nonkeydecoder
...
def main(args):
    ...
    if args.arch_decoder.endswith('deepsup'):
        segmentation_module = SegmentationModule(
            low_encoder, high_encoder, nonkey_match, nonkey_decoder, crit, args.deep_sup_scale)
    else:
        segmentation_module = SegmentationModule(
            low_encoder, high_encoder, nonkey_match, nonkey_decoder, crit)
    ...
    # Set up optimizers
    nets = (nonkey_match, nonkey_decoder, crit)
