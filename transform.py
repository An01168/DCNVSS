...
class Crop(object):
    ...
    def __call__(self, imgs, label):
        ...
        imgs_ = []
        for img in imgs:
            img = cv2.copyMakeBorder(img, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=self.padding)
...
class RandRotate(object):
    ...
    def __call__(self, imgs, label):
        if random.random() < self.p:
            ...
            imgs_ = []
            for img in imgs:
                img = cv2.warpAffine(img, matrix, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=self.padding)
                imgs_.append(img)
