...
class SegmentationModuleBase(nn.Module):
    ...
    def cal_similar(self, feature_map):
        reshape_feature_norm = torch.norm(feature_map, p=2, dim=1, keepdim= True) + 1e-10
        #reshape_feature_norm2 = reshape_feature_norm.repeat(1, feature_map.size(1), 1, 1)
        reshape_feature_norm3 = torch.div(feature_map, reshape_feature_norm)
        reshape_feature = reshape_feature_norm3.view(feature_map.size(0), feature_map.size(1), -1)
        similar = torch.matmul(reshape_feature.transpose(1, 2), reshape_feature)
        return similar


    def cal_losspa(self, org_similar_map, cur_similar_map):
        #similar_div = torch.pow(org_similar_map - cur_similar_map, 2)
        #similar_sum = torch.sum(similar_div)
        crit_fn = torch.nn.MSELoss(reduction='sum')
        similar_sum = crit_fn(org_similar_map, cur_similar_map)
        losspa = torch.div(similar_sum, org_similar_map.size(0)*org_similar_map.size(1)*org_similar_map.size(1))
        #print('losspa:',losspa)
        return losspa
...
class SegmentationModule(SegmentationModuleBase):
    def __init__(self, low_enc, high_enc, nonkey_mat, nonkey_dec, crit, deep_sup_scale=None):
        super(SegmentationModule, self).__init__()
        self.low_encoder = low_enc
        self.high_encoder = high_enc
        self.nonkey_match = nonkey_mat
        self.nonkey_decoder = nonkey_dec
        #self.encoder = net_enc
        #self.decoder = net_dec
        #self.teacher = net_tec
        self.crit = crit
    ...
    def forward(self, feed_dict, *, segSize=None):
        ...
        if segSize is None:
            if self.deep_sup_scale is not None: # use deep supervision technique
                ...
                key_highfea = self.high_encoder(key_lowfea).detach()
                nonkey_lowfea = self.low_encoder(feed_dict['cur_imgdata']).detach()
                local_atten = self.nonkey_match(key_lowfea, nonkey_lowfea)
                (transkey_highfea, pred) = self.nonkey_decoder(nonkey_lowfea, key_highfea, local_atten)
                nonkey_highfea = self.high_encoder(nonkey_lowfea).detach()
                nonkey_highfea = nn.functional.interpolate(nonkey_highfea, scale_factor=0.8, mode='bilinear').detach()
                nonkey_similar_map = self.cal_similar(nonkey_highfea).detach()

                transkey_highfea2 = nn.functional.interpolate(transkey_highfea, scale_factor=0.8, mode='bilinear')
                transkey_similar_map = self.cal_similar(transkey_highfea2)
                loss_pa = self.cal_losspa(nonkey_similar_map, transkey_similar_map)


            loss = self.crit(pred, feed_dict['seg_label']) + 20*loss_pa
            ...
            del nonkey_highfea, nonkey_similar_map
            return loss, acc
        # inference
        else:
